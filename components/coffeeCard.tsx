...
export default function CoffeeCard({ index, data }: Props) {
    ...
    return (
        <div className={`w-full coffeecard h-full relative rounded-xl mx-2 ${visible ? "opacity-100" : "opacity-0"}`} style={{animationDelay : `0.${index}s`}}>
            <div className='absolute top-0 left-0 z-20 w-full h-full p-1 rounded-xl'>
                <Image src={"/beans.jpg"} alt="coffee" width={1000} height={1000} className='object-cover w-full h-full opacity-50 rounded-xl' />
            </div>
            <div className='relative z-50 flex flex-col justify-between h-full p-4 bg-opacity-80 dark:bg-opacity-80 backdrop-blur-sm bg-orange-950 rounded-xl'>
                <div><div className='text-xl font-semibold text-white'>{data.name}</div>
                ...
                <div className='flex flex-row items-center justify-between w-full mt-4'>
                    <Link href={`./coffee/${encodeURI(data.name)}`} className='flex justify-between px-2 py-2 text-center text-orange-200 bg-orange-800 rounded-lg w-36 hover:bg-orange-700'>
                        <div>Learn More</div>
                    ...
                    <Link href='#' className='flex items-center justify-between w-32 px-2 py-2 ml-4 text-center rounded-lg bg-emerald-800 text-emerald-200 hover:bg-emerald-700'>
                        <div>Buy</div>
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
