...
export function TextField({ field, id }: Props) {
    return (
        <div className='flex flex-col'>
            ...
            <input type="text" name={id} id={id} className='p-2 mt-1 text-white bg-gray-800 rounded-lg' />
        </div>
    )
}
...
export function PasswordField({ field, id }: Props) {
    return (
        <div className='flex flex-col'>
            <label htmlFor={id}>{field}</label>
            <input type="password" name={id} id={id} className='p-2 mt-1 text-white bg-gray-800 rounded-lg' />
        </div>
    )
}
...
export function Google({ field }: Props) {
    return(
    <button className='mt-6 lg:w-[32rem] w-96 rounded-lg bg-coffee-300 hover:bg-coffee-400 items-center text-black flex flex-row justify-around px-2 py-3 font-semibold'>
