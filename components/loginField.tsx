...
export function Checkbox({ field, id }: Props) {
    return (
        <div className='flex w-full'>
            <input type="checkbox" name={id} id={id} className='accent-orange-800' />
            <label htmlFor={id} className='ml-2 select-none hover:text-orange-400'>{field}</label>
        </div>
    )
}
...
export function Google({ field }: Props) {
    return(
    <button className='mt-6 lg:w-[32rem] w-96 rounded-lg bg-orange-500 hover:bg-orange-600 items-center text-black flex flex-row justify-around px-2 py-3 font-semibold'>
